...
def test_config():
    assert isinstance(libreary.config, dict)
    assert libreary.config_dir is not None
...
def test_add_level():
    ...
    assert libreary.add_level("low", "1", levels_dict, copies=1) is None
...
def test_libreary_delete():
    obj_id = libreary.ingest(
        "test_run_dir/dropbox/grace.jpg",
        ["low"],
        "cat",
...
def test_search():
    o_id = libreary.ingest(
        "test_run_dir/dropbox/grace.jpg",
        ["low"],
        "Test File for Level System Design",
        delete_after_store=False)
...
def test_libreary_ingest_metadata():
    levels_dict = [
        {
            "id": "local1",
            "type": "LocalAdapter"
    ...
    assert libreary.add_level("low", "1", levels_dict, copies=1) is None
    obj_id = libreary.ingest("test_run_dir/dropbox/grace.jpg",
                             ["low"],
                             "cat",
                             metadata_schema=["collection", "owner"],
                             metadata=[{"field": "collection", "value": "cats"},
